...
def process_excel_file(file_path, salt_dict):
    ...
    # Iterate over the column 'student_studentNumber' and replace values with salted MD5 hashes
    column_name = 'student_studentNumber'
    df[column_name] = df.apply(lambda row: generate_md5_hash(row[column_name], salt_dict.get(row[column_name], '')), axis=1)
